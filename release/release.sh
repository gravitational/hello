...
PROJ_DIR=${GOPATH}/src/github.com/$ORG/${PROJ}
...
function setup_env {
	...
	if [ ! -d ${proj_dir} ]; then
		git clone https://$USERNAME:$PASSWORD@github.com/$ORG/${proj} ${proj_dir}
	fi
	...
	pushd ${proj_dir}
		...
		git checkout $BRANCH
        git tag $ver
        git push --tags --repo https://$USERNAME:$PASSWORD@github.com/$ORG/$proj
	popd
}
...
function main {
	...
	for os in linux; do
		...
		TARGET="${RELEASE_DIR}/${PROJ}-${VER}-${GOOS}-${GOARCH}"
		mkdir -p ${TARGET}
